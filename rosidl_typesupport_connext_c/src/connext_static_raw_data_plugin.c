    ...
    const struct DDS_TypeAllocationParams_t * alloc_params) 
{
    ...
    RTIOsapiHeap_allocateStructure(&(sample),ConnextStaticRawData);
    if (sample == NULL) {
        return NULL;
    }
...
ConnextStaticRawDataPluginSupport_create_key_ex(RTIBool allocate_pointers){
    ...
    RTIOsapiHeap_allocateStructure(&(key),ConnextStaticRawDataKeyHolder);
 ...
 * It expects the ConnextStaticRawData to contain data serialized with its
 ...
 *	  RTPSSerializedPayload or we are serializing as part of a nested type).
      For ConnextStaticRawDataPlugin_serialize() it must be set to TRUE
 *
 * @param encapsulation_id the encapsulation identifier in case serialize_encapsulation==TRUE
 *    the implementation of ConnextStaticRawDataPlugin_serialize checks that the value of
    ...
    void *endpoint_plugin_qos)
{
    ...
    if(serialize_sample) {
		 ...
		 * The ConnextStaticRawData sample->serialized_data is always a contiguous buffer
		 */
...
/**
	...
	stream size is alredy correct when ConnextStaticRawDataPlugin_deserialize_sample
	is called. 
    ...
    void *endpoint_plugin_qos)
{
    ...
    if (deserialize_sample) {
		/* Note that sample->key_hash was already set by ConnextStaticRawDataPlugin_deserialize()
		   it is done there because ConnextStaticRawDataPlugin_deserialize_sample does not 
		...
		*/
		...
		/* Do not call ConnextStaticRawData_initialize_ex initialize here 
...
ConnextStaticRawDataPlugin_get_key_kind_USER_KEY(void)
{
    return PRES_TYPEPLUGIN_USER_KEY;
}

PRESTypePluginKeyKind
ConnextStaticRawDataPlugin_get_key_kind_NO_KEY(void)
{
...
struct PRESTypePlugin * ConnextStaticRawDataPlugin_new_external(struct DDS_TypeCode * type_code)
{
    ...
    if ( (type_code != NULL) && DDS_TypeCode_is_type_keyed(type_code) ) {
      ...
      plugin->getKeyKindFnc = (PRESTypePluginGetKeyKindFunction)
        ConnextStaticRawDataPlugin_get_key_kind_NO_KEY;
    }
